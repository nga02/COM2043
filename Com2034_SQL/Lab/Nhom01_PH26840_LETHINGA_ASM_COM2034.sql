	...
	BEGIN TRY
			...
			ELSE
				...
				ELSE IF  NOT EXISTS(SELECT * FROM NGUOIDUNG WHERE MANGDUNG=@MANGDUNG)--KIỂM TRA KHOÁ NGOẠI
								...
								END
				ELSE IF   EXISTS(SELECT * FROM DANHGIA WHERE MANGDUNG=@MANGDUNG AND MANHATRO=@MANHATRO)--KIỂM TRA KHOÁ CHÍNH
								BEGIN
									SET @check3 = 1
									PRINT N'NGƯỜI DÙNG NÀY ĐÃ ĐÁNH GIÁ CHO NHÀ TRỌ NÀY'
								END
				ELSE IF  @check1 = 0 and @check2 = 0 and @check3 = 0 
					BEGIN
						INSERT INTO DANHGIA 
...
--GỌI THỦ TỤC
---GỌI THÀNH CÔNG
EXEC insert_DanhGia1 'T0049','N10',N'LIKE', N'Tốt'
...
PRINT N'Tổng số Like và Dislike của nhà trọ la : '+CAST(dbo.f_DanhGia('T1') AS VARCHAR(10))
...
GO
 CREATE PROC xoa_thongtin1 @count_dislike int
	...
	BEGIN
		...
		INSERT INTO @danhgia 
			SELECT MANHATRO FROM DANHGIA WHERE TRANGTHAI = N'DISLIKE' 
			GROUP BY MANHATRO 
...
EXEC xoa_thongtin2 @tgianbdau = '2022-05-15', 
                     @tgiankthuc = '2022-05-20' 
 --------------------
